                                       ...
                                       UINT8 pin_len, UINT8 *p_pin, UINT32 trusted_mask[])
{
...
#ifdef RDA_BT
    BD_ADDR  mini928_bd_addr={0x00,0x43,0x54,0x48,0x5a,0x45};
#endif
	...
	#ifdef RDA_BT
    if(memcmp (btm_cb.pairing_bda, mini928_bd_addr, BD_ADDR_LEN) == 0){
...
#ifdef RDA_BT
    p_dev_rec->sm4=BTM_SM4_TRUE;/*go to creat_connection directly ,to shorten first bonding time*/
#endif
    if ((btm_cb.security_mode == BTM_SEC_MODE_SP ||
         btm_cb.security_mode == BTM_SEC_MODE_SP_DEBUG ||
         btm_cb.security_mode == BTM_SEC_MODE_SC) &&
